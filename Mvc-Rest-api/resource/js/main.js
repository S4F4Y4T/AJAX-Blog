...
const BASE = 'https://safayat.a2hosted.com/app/api/';
    ...
    function fetch(page){
        $.ajax({
            ...
            success: res => {
                ...
                (res).forEach((post) => {
                    $('.post-container').html(
                        $('.post-container').html() + `<div class="post-body"><h3>${(post.title).substr(0,30)}<span class='remove' id='delete' postid=${post.postid}>Delete</span></h3><div class='post-block'><div class='img-box'><img src='https://safayat.a2hosted.com/images/${post.image}' alt=""></div><p>${(post.body).substr(0,200)}</p></div></div>`          
                    );
