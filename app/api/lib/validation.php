...
class validation{
	...
	public static function xss(){
		...
		self::$value[self::$currentvalue] = str_replace(["\r\n", "\r", "\n"], ["<br>", "<br>", "<br>"], self::$value[self::$currentvalue]);
		return __CLASS__;
	}

	public static function extention($data,$key){
		self::$value['name']  = $data[$key]['name'];
		self::$value['size']  = $data[$key]['size'];
		self::$value[$key]['tmp']  = $data[$key]['tmp_name'];
	...
	public static function image(){
		...
		if(empty(self::$value['name'])){
			self::$error = 'Must attach a image';
		}
	...
	public static function length($key,$min,$max){
	if(strlen(self::$value[self::$currentvalue]) < $min || strlen(self::$value[self::$currentvalue]) > $max){
			self::$error = "<div class='alert alert-danger'>".$key.":Length should be between ".$min."-".$max."</div>";
		}
		return __CLASS__;
	}
